...
def settings_path(path: str | Path | None = None) -> Path:
    return Path(path) if path is not None else writable_config_path(SETTINGS_FILE_NAME)
...
def settings_defaults_path(path: str | Path | None = None, *, settings_file: str | Path | None = None) -> Path:
    ...
    return writable_config_path(SETTINGS_DEFAULTS_FILE_NAME)


def admin_auth_path(path: str | Path | None = None) -> Path:
    return Path(path) if path is not None else writable_config_path(ADMIN_AUTH_FILE_NAME)
