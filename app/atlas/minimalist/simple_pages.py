...
class MinimalistSimpleContent(QWidget):
    ...
    def _render_settings(self) -> None:
        ...
        grid.setSpacing(12)
        cards = (
            ("Data Refresh", loaded_text, "Refreshes use the existing Atlas data loader.", "status"),
            ("PDF Output", str(pdf_folder), "Profile PDFs use the existing output folder.", "doc"),
            ("Search Index", "Refresh available", "Rebuild uses the existing data refresh path.", "library"),
        )
        ...
        refresh.clicked.connect(lambda: self.controller.refresh_data(force=True))
        ...
        rebuild.clicked.connect(lambda: self.controller.refresh_data(force=True))
