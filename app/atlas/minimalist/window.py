...
class MinimalistAtlasWindow(QMainWindow):
    ...
    def __init__(self, config: UserConfig, *, auto_refresh: bool = True, settings: AtlasSettings | None = None):
        ...
        self._minimalist_theme_preference = normalize_theme_preference(
            self.minimalist_app_settings.get("app", {}).get("theme")
            if isinstance(self.minimalist_app_settings, dict)
            else None
        )
        ...
        self._auto_refresh_timer.timeout.connect(lambda: self.refresh_data(force=True))
        ...
        self.pages = {
            ...
            "diagnostics": self.settings_page,
    ...
    def preview_minimalist_theme(
        ...
        *,
        ...
        app_settings = (
            self.minimalist_app_settings.get("app", {}) if isinstance(self.minimalist_app_settings, dict) else {}
        )
    ...
    def _apply_minimalist_behavior_settings(self) -> None:
        ...
        loading_settings = (
            self.minimalist_app_settings.get("data_loading", {})
            if isinstance(self.minimalist_app_settings, dict)
            else {}
        )
        ...
        auto_refresh = bool(loading_settings.get("auto_refresh_enabled", False)) and not bool(
            loading_settings.get("manual_refresh_only", False)
        )
        try:
        ...
        mysql_api_mode = os.getenv("EOAT_ATLAS_DATA_BACKEND", "legacy").strip().casefold() == "mysql_api"
        if (
            not mysql_api_mode
    ...
    def _configured_source_paths(self) -> dict[str, str]:
        ...
        if not isinstance(paths, dict):
            return {}
        return {str(key): str(value) for key, value in paths.items() if str(value or "").strip()}
    ...
    def _check_source_file_changes(self) -> None:
        ...
        if changed and bool(self.minimalist_setting("data_loading.warn_when_files_changed", True)):
            self.show_status("Source files changed since the last load. Save any Settings changes, then reload data.")
    ...
    def refresh_data(self, *, force: bool) -> None:
        ...
        self.fit_check_page.set_bundle(None)
        self.packet_builder_page.set_bundle(None)
        self.library_page.set_bundle(None)
        self.standards_page.set_bundle(None)
        self.settings_page.set_bundle(None)
        self.data_health_page.set_bundle(None)
    ...
    def _data_loaded(self, bundle: AtlasDataBundle) -> None:
        ...
        load_message = f"Loaded {len(bundle.eoats)} EOATs, {len(bundle.machines)} machines, {len(bundle.tools)} tools."
    ...
    def _data_failed(self, message: str) -> None:
        if self.bundle is not None and bool(self.minimalist_setting("data_loading.cache_last_good_data", True)):
            self._using_cached_data_fallback = True
            if bool(self.minimalist_setting("data_loading.show_cached_data_warning", True)):
                self.show_status(
                    f"Atlas data load failed; showing cached data from the last successful read. {message}"
                )
            self.data_ready.emit(self.bundle)
            return
    ...
    def show_page(self, key: str) -> bool:
        with perf_timer(
            ...
            if normalized in {"settings", "diagnostics"}:
                return self._activate_page(self.settings_page, "settings", "settings")
    ...
    def _activate_page(self, page: QWidget, page_key: str, nav_key: str) -> bool:
        ...
        transitions_enabled = (
            not bool(app_settings.get("reduce_motion", False))
            and str(app_settings.get("animation_speed", "smooth")) != "reduced"
        )
        previous_page_key = self.current_page_key
        if not self.page_transition.switch_to_widget(
            page, animated=bool(self.current_page_key) and transitions_enabled
        ):
            return False
    ...
    def search_entities(self, query: str, *, limit: int = 30):
        ...
        search_settings = (
            self.minimalist_app_settings.get("search", {}) if isinstance(self.minimalist_app_settings, dict) else {}
        )
        with perf_timer(
            self.config.project_root,
            ...
            raw_result = self._entity_search_index.search(
                text, limit=max(limit, int(search_settings.get("recent_items_limit", 15) or 15))
            )
            allowed_types = {
                entity_type
            ...
            current_scope = (
                self._current_library_scope() if bool(search_settings.get("prefer_current_library_type", True)) else ""
            )
            ...
            if current_scope or prefer_exact:
                ...
                def primary_exact(result) -> bool:
                    if not prefer_exact:
                        return False
                    return normalize_entity_id(result.entity_type, result.entity_id) == normalize_entity_id(
            ...
            results.extend(
                self._file_search_results(text, search_settings, limit=max(0, int(limit or 0)) - len(results))
            )
    ...
    def _file_search_results(self, query: str, search_settings: dict, *, limit: int) -> list[EntitySearchResult]:
        ...
        paths = self.minimalist_app_settings.get("paths", {}) if isinstance(self.minimalist_app_settings, dict) else {}
        output_folder = (
            Path(str(paths.get("output_folder") or "")).expanduser()
            if isinstance(paths, dict) and paths.get("output_folder")
            else None
        )
        reference_folder = (
            Path(str(paths.get("reference_docs_folder") or "")).expanduser()
            if isinstance(paths, dict) and paths.get("reference_docs_folder")
            else None
        )
        if bool(search_settings.get("include_setup_packets", False)) and output_folder is not None:
            candidates.extend(
                ("setup_packet", "Setup Packet", path) for path in self._iter_search_files(output_folder, {".pdf"})
            )
        ...
        *,
        ...
        if not text:
            ...
            self._log_search_resolution(
                resolved, source=source, navigation_occurred=False, recommendation_fallback=False
            )
            return resolved
        if self.bundle is None:
            self.show_status("Atlas data is still loading.")
        ...
        if resolved.found and resolved.entity_type in {"eoat", "machine", "tool"}:
            default_action = str(
                self.minimalist_setting("search.default_action", "open_best_match") or "open_best_match"
            )
            if default_action == "show_result_preview":
                self.show_status(
                    f"Best match: {resolved.display_label or resolved.entity_id}. Select it from search results to open."
                )
                self._log_search_resolution(
                    resolved, source=source, navigation_occurred=False, recommendation_fallback=False
                )
        ...
        if log_search and entity_type in {"eoat", "machine", "tool"}:
            self._log_search_resolution(
                SearchResolution(
                    ...
                    display_label=str(
                        getattr(entity, "display_label", "") or getattr(entity, "title", "") or entity_id
                    ),
                    route_target={"page": "library", "entity_type": entity_type, "entity_id": entity_id}
    ...
    def open_machine(self, machine: str, *, source: str = "") -> None:
        with perf_timer(
            ...
            self.show_status(
                f"Machine {record.machine}: {record.robot_type or record.robot_model or 'robot info missing'} | EOATs: {eoats}."
            )
    ...
    def record_minimalist_search(self, query: str, *, kind: str = "", update: bool = True) -> None:
        ...
        record_recent_search(
            ...
            kind=resolved_kind,
            bundle=self.bundle,
            limit=int(self.minimalist_setting("search.recent_items_limit", 15) or 15),
        )
        if update:
            self._refresh_recent_searches()
    ...
    def record_recent(self, item_type: str, key: str) -> None:
        ...
        if result is not None:
            record_recent_entity_result(
                result, query=text, limit=int(self.minimalist_setting("search.recent_items_limit", 15) or 15)
            )
        attr = {
            "eoat": "recent_eoats",
    ...
    def _log_search_resolution(
        ...
        *,
        ...
        payload = {
            ...
            or normalize_search_term(getattr(resolution, "raw_query", "")),
