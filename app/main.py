...
def main() -> int:
    ...
    config = load_config()
    log_performance(config.project_root, "app_start.load_config", time.perf_counter() - config_started)
    ...
    log_performance(config.project_root, "app_start.window_create", time.perf_counter() - window_started)
    window._single_instance_guard = instance_guard
    window.show()
    log_performance(config.project_root, "app_start.shell_visible", time.perf_counter() - startup_started)
