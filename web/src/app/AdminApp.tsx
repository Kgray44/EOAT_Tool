...
const filterNames = ["start", "end", "actor", "action", "action_category", "entity_type", "entity_id", "result", "source", "request_id", "correlation_id", "search", "current_user_changes", "security_events_only", "page", "page_size"];
...
function AuditFilters({ params, update, catalog }: { params: URLSearchParams; update: (name: string, value: string) => void; catalog?: AuditCatalog }) {
  return <section className="filters" aria-labelledby="filter-title"><div className="section-heading"><h2 id="filter-title">Investigate events</h2><button type="button" className="link-button" onClick={() => update("search", "")}>Clear search</button></div><div className="filter-grid"><label>Search safe evidence<input value={params.get("search") ?? ""} onChange={(e) => update("search", e.target.value)} placeholder="Event, actor, or entity" /></label><label>Start (UTC)<input type="datetime-local" value={params.get("start")?.slice(0, 16) ?? ""} onChange={(e) => update("start", e.target.value ? `${e.target.value}:00Z` : "")} /></label><label>End (UTC)<input type="datetime-local" value={params.get("end")?.slice(0, 16) ?? ""} onChange={(e) => update("end", e.target.value ? `${e.target.value}:00Z` : "")} /></label><Select label="Action" value={params.get("action") ?? ""} values={catalog?.actions} onChange={(value) => update("action", value)} /><Select label="Category" value={params.get("action_category") ?? ""} values={catalog?.action_categories} onChange={(value) => update("action_category", value)} /><Select label="Result" value={params.get("result") ?? ""} values={catalog?.results} onChange={(value) => update("result", value)} /><Select label="Source" value={params.get("source") ?? ""} values={catalog?.sources} onChange={(value) => update("source", value)} /><label>Actor<input value={params.get("actor") ?? ""} onChange={(e) => update("actor", e.target.value)} /></label><label>Entity type<input value={params.get("entity_type") ?? ""} onChange={(e) => update("entity_type", e.target.value)} /></label><label>Entity ID<input value={params.get("entity_id") ?? ""} onChange={(e) => update("entity_id", e.target.value)} /></label><label>Request ID<input value={params.get("request_id") ?? ""} onChange={(e) => update("request_id", e.target.value)} /></label><label>Correlation ID<input value={params.get("correlation_id") ?? ""} onChange={(e) => update("correlation_id", e.target.value)} /></label></div><div className="preset-row"><button type="button" onClick={() => update("start", new Date(Date.now() - 3600_000).toISOString())}>Last hour</button><button type="button" onClick={() => update("start", new Date(Date.now() - 86_400_000).toISOString())}>Last 24 hours</button><button type="button" onClick={() => update("start", new Date(Date.now() - 7 * 86_400_000).toISOString())}>Last 7 days</button><button type="button" onClick={() => update("result", "FAILURE")}>Failed</button><button type="button" onClick={() => update("result", "DENIED")}>Denied</button><button type="button" onClick={() => update("security_events_only", params.get("security_events_only") === "true" ? "" : "true")}>Security / authentication</button></div></section>;
}

function Select({ label, value, values, onChange }: { label: string; value: string; values?: string[]; onChange: (value: string) => void }) { return <label>{label}<select value={value} onChange={(e) => onChange(e.target.value)}><option value="">All</option>{(values ?? []).map((option) => <option key={option} value={option}>{option.replaceAll("_", " ")}</option>)}</select></label>; }
