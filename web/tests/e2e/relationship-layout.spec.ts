...
const relationship = (type: "eoat" | "tool", index: number) => ({
  ...
  reason:
    index % 2 ? "VERIFIED_SOURCE" : "OBSERVATION_OR_LATER_LIFECYCLE_EVENT",
});
...
test("relationship cards remain compact, centered, responsive, and semantically readable", async ({
  page,
}) => {
  ...
  for (const [count, expected] of [
    ...
    await routeProfile(
      ...
      Array.from({ length: count }, (_, index) =>
        relationship(index % 2 ? "tool" : "eoat", index),
      ),
    );
    ...
    if (count === 0) {
      await expect(
        page.getByText("No current EOAT assignment observed"),
      ).toBeVisible();
    }
    if (count === 1) {
    ...
    if (count >= 3) {
      await expect(
        page.getByText("Historical observation").first(),
      ).toBeVisible();
      await expect(
        page.getByText("Verified compatibility").first(),
      ).toBeVisible();
    }
  }
