...
function Write-ScheduledToolLog {
    ...
    try {
        ...
        Write-EmergencyLog -Line "[$timestamp] NORMAL_LOG_WRITE_FAILED weekly_summary root=`"$Root`" error=`"$($_.Exception.Message -replace '"', "'")`" line=$Line"
        return $false
    }
}
...
function Get-WeeklyCreatedReportPath {
    param($Output)

    foreach ($line in $Output) {
        if ([string]$line -match "Weekly summary saved to\s+(.+?)(?:\.)?$") {
...
function Get-WeeklyExistingReportPath {
    param($Output)

    foreach ($line in $Output) {
        if ([string]$line -match "Existing weekly summary:\s*(.+?)(?:\.)?$") {
            return (ConvertTo-NormalFileSystemPath -PathValue $Matches[1].Trim().Trim('"'))
        }
    }
    return ""
...
try {
    ...
    $script = Resolve-NormalPath -PathValue (Join-Path $appRoot "tools\weekly_summary_generator.py") -RequireExists
    $launchArgs = @(
        $script,
    ...
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    Write-ScheduledToolLog -Root $projectRootResolved -Line "[$timestamp] START weekly_summary scheduled=true command=`"$($commandLine -replace '"', "'")`"" | Out-Null
    ...
    if ($exitCode -ne 0) {
        ...
        $line = "[$timestamp] FAILURE weekly_summary exit_code=$exitCode elapsed=${elapsed}s error=`"$safeOutput`""
        Write-ScheduledToolLog -Root $projectRootResolved -Line $line | Out-Null
        Write-EmergencyLog -Line $line
        exit $exitCode
    }

    $createdReport = Get-WeeklyCreatedReportPath -Output $output
    $existingReport = Get-WeeklyExistingReportPath -Output $output
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    if ($createdReport -and (Test-Path -LiteralPath $createdReport)) {
        $line = "[$timestamp] SUCCESS weekly_summary report_created=true output=`"$createdReport`" elapsed=${elapsed}s"
        Write-ScheduledToolLog -Root $projectRootResolved -Line $line | Out-Null
        exit 0
    }
    if ($existingReport -and (Test-Path -LiteralPath $existingReport)) {
        $line = "[$timestamp] SKIPPED weekly_summary report_created=false reason=`"report already exists`" output=`"$existingReport`" elapsed=${elapsed}s"
        Write-ScheduledToolLog -Root $projectRootResolved -Line $line | Out-Null
        exit 0
    }

    $safeOutput = $outputText -replace '"', "'"
    $line = "[$timestamp] FAILURE weekly_summary report_created=false elapsed=${elapsed}s error=`"python exited 0 but no report file was confirmed. Output: $safeOutput`""
    Write-ScheduledToolLog -Root $projectRootResolved -Line $line | Out-Null
    Write-EmergencyLog -Line $line
    exit 1
} catch {
