...
from release_tools.launcher import update_and_launch
...
def main() -> int:
    ...
    if state["version"] != "0.9.1":
        raise AssertionError("Packaged launcher did not install 0.9.1")
    ...
    def smoke_launch(exe: Path) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="eoat_manual_smoke_") as runtime:
            env = os.environ.copy()
            env.update(
                EOAT_ATLAS_SMOKE_TEST="1",
                EOAT_ATLAS_SMOKE_RUNTIME_PROBE="1",
                EOAT_ATLAS_LOCALAPPDATA=runtime,
    ...
    if (
        action_bad != "update-failed-fallback"
        or json.loads(current.read_text())["version"] != "0.8.0"
        or len(launches) != 3
    ):
    ...
    if current_files != ["EOAT-Atlas_v0.9.1.zip"]:
        raise AssertionError(f"Current package folder is not clean: {current_files}")
    print(
        json.dumps(
            {
                "packaged_launcher_install": "passed",
                "equal_no_redownload": "passed",
                "app_smoke_launches": len(launches),
                "offline_fallback": "passed",
                "bad_checksum_fallback": "passed",
