[CmdletBinding()]
param([Parameter(Mandatory=$true)][string]$Revision, [string]$DatabaseName = "eoat_atlas_dev")
...
& ".\.venv\Scripts\python.exe" -m alembic -c "server\alembic.ini" downgrade $Revision
exit $LASTEXITCODE
