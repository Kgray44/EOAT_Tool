...
The exact canonical command `python run_atlas.py` completed its smoke-validation launch successfully from the canonical network path. All captured Python module paths were inside the canonical repository. The banner reported application 0.9.1, API 1.3.0, MySQL 8.4.9, `eoat_atlas_dev`, schema `20260714_0004`, `mysql_api`, and disabled legacy fallback.
