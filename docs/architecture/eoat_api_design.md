...
Write groups cover assets, compatibility, transactional location moves, audits, maintenance, documents, photo metadata, tags, annotations, application instances, structured EOAT history, and the Phase 1 global audit ledger. API version is `1.4.0`; expected schema revision is `20260811_0005`. Errors use `error_code`, `message`, `details`, `request_id`, `timestamp`, `retryable`, and `current_record_version`. Database details and secrets are never returned.
