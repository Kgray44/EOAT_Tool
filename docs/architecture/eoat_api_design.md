...
Write groups cover assets, compatibility, transactional location moves, audits, maintenance, documents, photo metadata, tags, annotations, application instances, and structured EOAT history. API version is `1.3.0`; expected schema revision is `20260714_0004`. Errors use `error_code`, `message`, `details`, `request_id`, `timestamp`, `retryable`, and `current_record_version`. Database details and secrets are never returned.
