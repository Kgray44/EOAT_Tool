...
| Audit Context + Physical Audit Verified + Press/Machine # | Latest observed installed state | audit_records plus future sanctioned location observation | details_json; no safe normalized destination at revision 20260715_0006 | Only an `Audited`, physically verified `Installed on Machine` row may establish observed installation. Compatibility rows never do. Do not map the audit date to the original `installed_at`. | Yes, pending observation-time schema semantics |
| Notes containing `EOAT in cabinet` and `EOAT Not Installed` | Latest observed stored state | audit_records plus future sanctioned location observation/storage assignment | notes/details_json; no safe normalized destination when cabinet and `stored_at` are unknown | Explicit EOAT cabinet note overrides generic machine/context. Preserve unspecified cabinet honestly; do not infer from a tool-in-cabinet note. | Yes, pending sanctioned generic storage/observation semantics |
