...
| Normal-route browser continuation | PASS for direct EOAT/Machine/Tool routes and a Machine refresh against the real API. EOAT Event Detail `612895d1-de51-49ef-88a2-3bfc38fdc88d` opened `/eoats/P3-15b5838285-A`, rendered the committed profile value, and browser Back returned to the same Event Detail. Follow-up `3d2efb0d9c` preserves client-side rehearsal state across Admin navigation while keeping full-refresh CSRF behavior fail-closed. |
| Safe secret replacement receipt | PASS: browser DOM, console, response, and Event Detail did not contain the synthetic marker. Follow-up `33ca06eacd` makes an already-configured sensitive-setting replacement reconstructable as `replacement_recorded: false -> true`; the real-MySQL suite passed **12/12** after the correction. |
