...
| MySQL and schema                         | The accepted test path is Debian MySQL through the protected loopback SSH tunnel, with `eoat_atlas_test` at revision `20260813_0008`.                                                                                         | New operational state requires live acceptance only against `eoat_atlas_test`. No local Windows MySQL, development schema, or production schema may be used.                      |
...
The existing test runtime principal retains its prior read access and received
only these additional DML permissions in `eoat_atlas_test`:

| Table                      | Granted DML        | Why it is required                                                                                                                                                                                                                            |
| -------------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `admin_danger_step_ups`    | `INSERT`           | Issue an immutable, server-side scoped step-up proof.                                                                                                                                                                                         |
| `admin_operations`         | `INSERT`, `UPDATE` | Persist and advance an operation lifecycle; Phase 4 never deletes operation evidence.                                                                                                                                                         |
| `admin_operation_fixtures` | `INSERT`, `DELETE` | Seed controlled acceptance fixtures and remove only the exact approved fixture namespace during the recovery rehearsal. `DELETE` is required by the bounded `danger_commit` implementation and is not granted on any business or audit table. |

There is no schema-wide DML, DDL, `GRANT OPTION`, production-schema access, or
audit-ledger write permission in this change. The diagnostic guard checks this
exact three-table shape and fails the operation path closed when it is absent.
Any broader runtime database privilege, production-like recovery, or
browser-executable operational capability is a Phase 6 prerequisite requiring
a separately approved authority, scope, and acceptance run.
