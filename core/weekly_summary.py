...
def _workbook_metrics(project_root: str | Path) -> dict[str, Any]:
    ...
    return {
        ...
        "Pilot candidates flagged": data.metrics.get("pilot_candidate_yes_count", 0)
        + data.metrics.get("pilot_candidate_maybe_count", 0),
...
def build_weekly_summary_markdown(
    ...
    lines.append(
        "- Evidence basis: schedule status, daily reports, activity logs, workbook metrics, open items, and validation output where available."
    )
    if metrics:
        lines.append(
            "- Audit/data metrics are audit-observed or workbook-derived; no production impact is claimed from them alone."
        )
    else:
    ...
    if validation.get("available"):
        ...
        lines.append(
            "- Validation findings are missing; run workbook validation before relying on release readiness claims."
        )
    if open_items:
