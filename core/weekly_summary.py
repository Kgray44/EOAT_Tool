...
def generate_weekly_summary(project_root: str | Path, week: int, notes: str = "") -> ToolResult:
    start = time.perf_counter()
    paths = resolve_project_paths(project_root)
    ensure_directory(paths.weekly_reports)
    markdown, warnings, metrics = build_weekly_summary_markdown(project_root, week, notes)
    path = paths.weekly_reports / f"Week{week}_Summary_{time.strftime('%Y-%m-%d')}.md"
    if path.exists():
        path = paths.weekly_reports / f"Week{week}_Summary_{time.strftime('%Y-%m-%d_%H%M%S')}.md"
