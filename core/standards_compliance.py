...
class AuditComplianceResult:
    ...
    def recommended_actions(self) -> tuple[str, ...]:
        actions = [result.recommended_action for result in self.category_results if result.recommended_action and result.status != STATUS_COMPLIANT]
...
class StandardsComplianceSummary:
    ...
    def to_markdown(self) -> str:
        ...
        lines = [
            ...
            "",
            "## Audit Compliance",
            *table_from_rows(rows, ["Audit ID", "Press/Machine #", "EOAT Type", "Score", "Fails", "Warnings", "Unknown"]),
            "",
            "## Press / Cell Rollup",
            *table_from_rows(rollups, ["Press/Machine #", "Audits", "Average Score", "Worst Category", "Open Standards Issues", "Pilot Relevance"]),
            "",
            "## Method Notes",
...
def analyze_standards_compliance(project_root: str | Path) -> tuple[StandardsComplianceSummary | None, ToolResult | None]:
    paths = resolve_project_paths(project_root)
    if not paths.master_workbook.exists():
        return None, ToolResult.fail("standards_compliance", "Standards Compliance Analysis", "Master workbook is missing.", errors=[str(paths.master_workbook)])
    try:
        rows = row_dicts(paths.master_workbook, "EOAT Inventory")
    except Exception as exc:
        return None, ToolResult.fail("standards_compliance", "Standards Compliance Analysis", "Could not read EOAT Inventory.", errors=[str(exc)])
...
def rollup_compliance_by_press(audits: tuple[AuditComplianceResult, ...] | list[AuditComplianceResult]) -> list[PressComplianceRollup]:
...
def generate_standards_compliance_report(project_root: str | Path, *, log_activity: bool = True) -> ToolResult:
    ...
    try:
        ...
        return ToolResult.fail("standards_compliance", "Standards Compliance Analysis", "Could not write compliance report.", errors=[str(exc)])
...
def _classification_complete(row: dict[str, Any]) -> ComplianceCategoryResult:
    ...
    if _unknown(eoat_type):
        return _result("eoat_classification_complete", "EOAT classification complete", STATUS_UNKNOWN, "EOAT Type is unknown or blank.", "Verify the physical EOAT type.", ("EOAT Type",))
    return _result("eoat_classification_complete", "EOAT classification complete", STATUS_COMPLIANT, f"EOAT Type is documented as {eoat_type}.", "", ("EOAT Type",))


def _tooling_details_complete(row: dict[str, Any]) -> ComplianceCategoryResult:
    if _unknown(row.get("EOAT Type")):
        return _result("tooling_details_complete", "Tooling details complete", STATUS_UNKNOWN, "Tooling requirements depend on unknown EOAT type.", "Verify EOAT Type before judging tooling details.", ("EOAT Type",))
    ...
    if missing:
        return _result("tooling_details_complete", "Tooling details complete", STATUS_WARNING, f"Missing or unknown tooling fields: {', '.join(missing)}.", "Complete applicable tooling details or mark true N/A through the audit workflow.", tuple(required))
    return _result("tooling_details_complete", "Tooling details complete", STATUS_COMPLIANT, "Applicable tooling details are documented.", "", tuple(required))
...
def _pneumatic_routing(row: dict[str, Any]) -> ComplianceCategoryResult:
    if not (eoat_type_uses_vacuum(row) or eoat_type_uses_gripper(row)):
        return _result("pneumatic_routing", "Pneumatic routing condition", STATUS_NOT_APPLICABLE, "Pneumatic routing is not applicable to this EOAT classification.", "", ("Tubing Condition",))
...
def _sensor_standards(row: dict[str, Any]) -> ComplianceCategoryResult:
    ...
    fields = ("Sensors Present?", "Sensor Type", "Sensor Brand/Model", "Part-Present Detection Present?", "Vacuum Confirmation Present?")
    if _unknown(present):
        return _result("sensor_standards", "Sensor standard/documentation", STATUS_UNKNOWN, "Sensor presence is unknown.", "Verify whether sensors are present.", fields)
    if _no(present):
        return _result("sensor_standards", "Sensor standard/documentation", STATUS_NOT_APPLICABLE, "Sensors are documented as not present.", "", fields)
    missing = [field for field in fields[1:] if _unknown(row.get(field)) and not is_na_value(row.get(field))]
    if missing:
        return _result("sensor_standards", "Sensor standard/documentation", STATUS_WARNING, f"Sensor details need review: {', '.join(missing)}.", "Document sensor type, model, and confirmation method.", fields)
    return _result("sensor_standards", "Sensor standard/documentation", STATUS_COMPLIANT, "Sensor presence and detail fields are documented.", "", fields)
...
def _quick_disconnect_standards(row: dict[str, Any]) -> ComplianceCategoryResult:
    ...
    if _unknown(present):
        return _result("quick_disconnect_standards", "Quick disconnect standard", STATUS_UNKNOWN, "Quick disconnect presence is unknown.", "Verify whether quick disconnects are present.", fields)
    if _no(present):
        return _result("quick_disconnect_standards", "Quick disconnect standard", STATUS_NOT_APPLICABLE, "Quick disconnects are documented as not present.", "", fields)
    if present.casefold() == "partial":
        return _result("quick_disconnect_standards", "Quick disconnect standard", STATUS_WARNING, "Quick disconnects are only partially documented.", "Identify pneumatic/electrical quick disconnect standards or document the exception.", fields)
    if not any(is_meaningful_value(row.get(field)) for field in fields[1:]):
        return _result("quick_disconnect_standards", "Quick disconnect standard", STATUS_WARNING, "Quick disconnects are present but type fields are missing.", "Document the applicable quick disconnect type(s).", fields)
    return _result("quick_disconnect_standards", "Quick disconnect standard", STATUS_COMPLIANT, "Quick disconnect presence and type information are documented.", "", fields)
...
def _mechanical_mounting(row: dict[str, Any]) -> ComplianceCategoryResult:
    ...
    if bad:
        return _result("mechanical_mounting", "Mechanical mounting", STATUS_FAIL, f"Mechanical mounting/alignment issue fields: {', '.join(bad)}.", "Inspect mounting, locking hardware, and alignment before approving the standard.", fields)
    if unknown:
        return _result("mechanical_mounting", "Mechanical mounting", STATUS_UNKNOWN, f"Mechanical mounting fields are unknown: {', '.join(unknown)}.", "Verify mechanical mounting and alignment condition.", fields)
    return _result("mechanical_mounting", "Mechanical mounting", STATUS_COMPLIANT, "Mechanical mounting and alignment fields are documented without flagged issues.", "", fields)
...
def _safety_concerns(row: dict[str, Any]) -> ComplianceCategoryResult:
    ...
    if _yes(row.get("Follow-Up Needed")) or _yes(row.get("Scrap/Quality Concern?")):
        return _result("safety_concerns", "Safety concerns", STATUS_WARNING, "Follow-up, scrap, or quality concern is flagged.", "Review safety/quality risk before closing this audit.", fields)
    if issue and issue.casefold() not in {"none", "no", "n/a", "na", "no issue observed.", "no issues observed", "unknown / not checked", "unknown"}:
        status = STATUS_FAIL if "safety" in issue.casefold() else STATUS_WARNING
        return _result("safety_concerns", "Safety concerns", status, "Known Issues contains a documented concern.", "Review the issue and decide whether it belongs in open items or FMEA.", fields)
    if _unknown(issue):
        return _result("safety_concerns", "Safety concerns", STATUS_UNKNOWN, "Known Issues has not been checked.", "Confirm whether safety or reliability concerns exist.", fields)
    return _result("safety_concerns", "Safety concerns", STATUS_COMPLIANT, "No documented safety concern in current audit fields.", "", fields)
...
def _documentation_completeness(row: dict[str, Any]) -> ComplianceCategoryResult:
    ...
    if not missing:
        return _result("documentation_completeness", "Documentation completeness", STATUS_COMPLIANT, "Documentation status fields are complete or partial where allowed.", "", fields)
    unknown = [field for field in missing if _unknown(row.get(field))]
    if unknown:
        return _result("documentation_completeness", "Documentation completeness", STATUS_UNKNOWN, f"Documentation status unknown: {', '.join(unknown)}.", "Verify documentation availability rather than treating blanks as complete.", fields)
    return _result("documentation_completeness", "Documentation completeness", STATUS_FAIL, f"Documentation gaps remain: {', '.join(missing)}.", "Find documentation or record the gap as intentional follow-up.", fields)
...
def _pm_readiness(row: dict[str, Any]) -> ComplianceCategoryResult:
    ...
    if _unknown(row.get("Maintenance Frequency")):
        return _result("pm_readiness", "PM readiness", STATUS_UNKNOWN, "Maintenance frequency is unknown.", "Confirm PM frequency or document why it is unavailable.", fields)
    if _no(row.get("Process Binder Complete?")):
        return _result("pm_readiness", "PM readiness", STATUS_WARNING, "Process binder is not complete.", "Complete binder references before calling PM readiness complete.", fields)
    return _result("pm_readiness", "PM readiness", STATUS_COMPLIANT, "Maintenance frequency and binder readiness are documented.", "", fields)
...
def _bom_spare_parts_readiness(row: dict[str, Any]) -> ComplianceCategoryResult:
    ...
    if _unknown(row.get("BOM Available?")) or _unknown(row.get("Spare Parts Identified?")):
        return _result("bom_spare_parts_readiness", "BOM/spare parts readiness", STATUS_UNKNOWN, "BOM or spare parts status is unknown.", "Verify BOM and spare-parts status.", fields)
    if not is_bom_available(row) or is_spare_parts_info_missing(row):
        return _result("bom_spare_parts_readiness", "BOM/spare parts readiness", STATUS_FAIL, "BOM and spare-parts readiness is not complete.", "Confirm BOM and spare-parts availability before standardization handoff.", fields)
    return _result("bom_spare_parts_readiness", "BOM/spare parts readiness", STATUS_COMPLIANT, "BOM and spare-parts readiness fields are documented as available.", "", fields)
...
def _photo_evidence_readiness(project_root: str | Path, row: dict[str, Any]) -> ComplianceCategoryResult:
    ...
    if coverage is None:
        return _result("photo_evidence_readiness", "Photo evidence readiness", STATUS_UNKNOWN, "Photo evidence coverage could not be evaluated.", "Create or load an audit row before judging photo evidence.", ("Photos Taken?", "Photo Folder/Link"))
    ...
    if missing:
        return _result("photo_evidence_readiness", "Photo evidence readiness", STATUS_FAIL, f"Required photo evidence missing: {', '.join(missing)}.", "Capture and intake the missing local photo evidence.", ("Photos Taken?", "Photo Folder/Link"))
    if follow_up:
        return _result("photo_evidence_readiness", "Photo evidence readiness", STATUS_WARNING, f"Recommended evidence still needs follow-up: {', '.join(follow_up[:4])}.", "Capture recommended evidence where practical.", ("Photos Taken?", "Photo Folder/Link"))
    return _result("photo_evidence_readiness", "Photo evidence readiness", STATUS_COMPLIANT, "Required photo evidence categories are covered or not applicable.", "", ("Photos Taken?", "Photo Folder/Link"))
...
def _condition_result(
    ...
    *,
    ...
    if _unknown(text):
        return _result(key, label, STATUS_UNKNOWN, f"{field_name} is unknown or blank.", f"Verify {field_name}.", (field_name,))
    if folded in good:
    ...
    if any(token in folded for token in fail_tokens):
        return _result(key, label, STATUS_FAIL, f"{field_name} indicates a standards failure: {text}.", action, (field_name,))
    ...
    return _result(key, label, STATUS_WARNING, f"{field_name} value is documented but not recognized as verified compliant: {text}.", f"Review {field_name} against the local standard.", (field_name,))
...
def _unknown(value: Any) -> bool:
    text = _text(value)
    folded = text.casefold()
    return not text or folded in {"unknown / not checked", "unknown", "not checked", "unknown / needs review"} or folded.startswith("unknown")
