...
def _files(
    folder: Path, patterns: tuple[str, ...] = ("*.md", "*.docx", "*.pdf", "*.xlsx", "*.csv", "*.png", "*.pptx")
) -> list[Path]:
...
def check_deliverables(project_root: str | Path) -> tuple[list[DeliverableStatus], list[str]]:
    ...
    statuses.append(
        DeliverableStatus(
            ...
            "Found" if guideline_files else "Missing",
            [str(p) for p in guideline_files[:3]],
            "Looked in 03_Standards/EOAT_Design_Guideline_Draft.",
        )
    )
    ...
    statuses.append(
        DeliverableStatus(
            "PM checklist",
            pm_status,
            [str(p) for p in (pm_files or template_files)[:3]],
            "Generated checklists count as found; templates alone are partial.",
        )
    )
    ...
    statuses.append(
        DeliverableStatus(
            ...
            fmea_status,
            [str(p) for p in fmea_reports[:3]],
            f"{len(fmea_rows)} FMEA Draft row(s).",
        )
    )
    ...
    pilot_status = (
        "Found" if pilot_reports and before_after else ("Partial" if pilot_reports or before_after else "Missing")
    )
    statuses.append(
        DeliverableStatus(
            "Pilot optimization project/report",
            pilot_status,
            [str(p) for p in (pilot_reports + before_after)[:3]],
            "Before/after data is required before claiming pilot results.",
        )
    )
    ...
    statuses.append(
        DeliverableStatus(
            ...
            kpi_status,
            [str(p) for p in kpi_reports[:3]],
            f"{len(kpi_rows)} KPI Baseline row(s).",
        )
    )
    ...
    statuses.append(
        DeliverableStatus(
            "Training materials",
            "Found" if training_files else "Missing",
            [str(p) for p in training_files[:3]],
            "Looked in final handoff training and standards work instruction folders.",
        )
    )
    ...
    statuses.append(
        DeliverableStatus(
            ...
            "Found" if bom_reports else "Missing",
            [str(p) for p in bom_reports[:3]],
            "BOM analysis reports are evidence; verified part numbers still require source confirmation.",
        )
    )
    ...
    statuses.append(
        DeliverableStatus(
            ...
            "Found" if doc_gap else "Missing",
            [str(p) for p in doc_gap[:3]],
            "Looked in Documentation_Gap_Reports.",
        )
    )

    package_exec_files = (
        [path for path in paths.final_handoff.glob("Final_Handoff_Package_*/Executive_Summary.md") if path.is_file()]
        if paths.final_handoff.exists()
        else []
    )
    exec_files = _files(paths.executive_summary) + sorted(
        package_exec_files, key=lambda path: path.stat().st_mtime, reverse=True
    )
    statuses.append(
        DeliverableStatus(
            "Executive summary",
            "Found" if exec_files else "Missing",
            [str(p) for p in exec_files[:3]],
            "Looked in 06_Final_Handoff/Executive_Summary and Phase 11 packages.",
        )
    )
    ...
    statuses.append(
        DeliverableStatus(
            ...
            "Found" if pres_packages else "Missing",
            [str(p) for p in pres_packages[:3]],
            "Looked for Auto_Exported_Content packages.",
        )
    )

    recommendation_files = _files(paths.final_report) + _files(
        paths.presentation_assets_root, ("final_recommendations.md", "*.md")
    )
    statuses.append(
        DeliverableStatus(
            "Final recommendations",
            "Found" if recommendation_files else "Missing",
            [str(p) for p in recommendation_files[:3]],
    ...
    handoff_packages = sorted(
        [*phase11_packages, *legacy_packages], key=lambda path: path.stat().st_mtime, reverse=True
    )
    statuses.append(
        DeliverableStatus(
            "Handoff package",
            "Found" if handoff_packages else "Missing",
            [str(p) for p in handoff_packages[:3]],
            "Looked in 06_Final_Handoff for Phase 11 packages and legacy Handoff_Package outputs.",
        )
    )
...
def _markdown(statuses: list[DeliverableStatus], warnings: list[str]) -> str:
    counts = {
        ...
        for status in ["Found", "Partial", "Needs review", "Missing", "Not applicable"]
    }
...
def run_final_deliverable_check(project_root: str | Path, log_activity: bool = True) -> ToolResult:
    ...
    report = write_timestamped_report(
        paths.handoff_package_root, "Final_Deliverable_Check", _markdown(statuses, warnings)
    )
    metrics = {
        status: sum(1 for item in statuses if item.status == status)
        for status in {"Found", "Partial", "Needs review", "Missing", "Not applicable"}
