    ...
    def to_markdown(self) -> str:
        ...
        return "\n".join(
            [
                ...
                *([f"- {warning}" for warning in self.missing_data_warnings] if self.missing_data_warnings else ["No missing-data warnings."]),
                "",
...
def analyze_kpis(project_root: str | Path) -> tuple[KpiSummary | None, ToolResult | None]:
    ...
    missing_data_warnings = [label.missing_data_warning for label in truth_labels if label.missing_data_warning != "None obvious"]
    if not kpis:
    ...
    summary = KpiSummary(
        metrics={
            ...
            "measured_record_count": sum(label.source_breakdown.get("actual measured data", 0) for label in truth_labels),
            "audit_observed_record_count": sum(label.source_breakdown.get("audit-observed data", 0) for label in truth_labels),
            "estimated_subjective_record_count": sum(label.source_breakdown.get("estimated/subjective data", 0) for label in truth_labels),
...
def _classify_kpi_source(row: dict[str, Any], metric: str) -> str:
    text = " ".join(
        ...
        for field in ["Data Source", "Notes", "Maintenance Notes", "Scrap Reason"]
    ).casefold()
    value_text = str(row.get(metric) or "").casefold()
    combined = f"{text} {value_text}"
    if any(token in combined for token in ["estimate", "estimated", "approx", "about ", "~", "rough", "subjective", "assumed", "guess"]):
        return "estimated/subjective data"
    if any(token in combined for token in ["mes", "plc", "historian", "counter", "meter", "sensor", "measured", "automatic", "export", "system", "shift log extract"]):
        return "actual measured data"
    if any(token in combined for token in ["audit", "observed", "manual", "operator", "technician", "maintenance", "interview", "shift notes", "visual"]):
