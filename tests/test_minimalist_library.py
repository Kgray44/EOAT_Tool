...
from core.atlas_record_details import RecordDetailData, RecordPhoto, RecordPhotoGroup
...
def test_record_pdf_export_runs_in_background_and_restores_feedback(qapp, tmp_path: Path, monkeypatch) -> None:
    page = MinimalistLibraryContent(_Controller())
    page.resize(1400, 900)
    page.set_bundle(_library_bundle(tmp_path))
    ...
    def fake_export(detail_data, output_path=None, *, project_root=""):
        time.sleep(0.05)
        finished.set()
        return tmp_path / "exported.pdf"
    ...
    assert view.hero.export_button.text() == "Exporting..."
...
def test_machine_current_eoat_display_is_not_false_warning() -> None:
    ...
    assert machine_current_eoat_display(unknown).value == "Not Indexed"
    assert machine_current_eoat_display(unknown).state == "unknown"
    assert machine_current_eoat_display(explicit_none).value == "No Current EOAT"
...
def test_record_back_button_lives_in_top_chrome_and_returns_to_library(qapp, tmp_path: Path) -> None:
