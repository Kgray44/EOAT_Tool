...
def test_backup_discovery_reports_counts_by_source(fake_project, monkeypatch):
    ...
    _write_backup(paths.project_admin / "Backups" / "Workbook_Backups" / "EOAT_Master_Tracker_backup_20260501_080000.xlsx", now, 10)
...
def test_backup_retention_selects_old_excess_candidates(fake_project, monkeypatch):
    ...
    summary = summarize_backups(fake_project, BackupRetentionPolicy(keep_recent_days=7, keep_last_per_workbook=25), now=now)
...
def test_backup_cleanup_dry_run_and_confirmation(fake_project, monkeypatch):
    ...
    now = datetime(2026, 5, 28, 12, 0)
    folder = resolve_project_paths(fake_project).project_admin / "Backups" / "Workbook_Backups"
    backups = [_write_backup(folder / f"EOAT_Master_Tracker_backup_202604{index + 1:02d}_080000.xlsx", now, 20 + index) for index in range(27)]

    dry = cleanup_old_backups(fake_project, BackupRetentionPolicy(keep_recent_days=7, keep_last_per_workbook=25), dry_run=True)
    refused = cleanup_old_backups(fake_project, BackupRetentionPolicy(keep_recent_days=7, keep_last_per_workbook=25), dry_run=False, confirm=False)
    ...
    applied = cleanup_old_backups(fake_project, BackupRetentionPolicy(keep_recent_days=7, keep_last_per_workbook=25), dry_run=False, confirm=True)
...
def test_backup_cleanup_refuses_when_validation_has_blockers(fake_project, monkeypatch):
    ...
    result = cleanup_old_backups(fake_project, BackupRetentionPolicy(keep_recent_days=7, keep_last_per_workbook=0), dry_run=False, confirm=True)
