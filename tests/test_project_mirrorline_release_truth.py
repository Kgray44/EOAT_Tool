...
def test_exhaustive_project_mirrorline_review_has_one_finalized_patch_release() -> None:
    ...
    finalized_0232 = [
        entry
        for entry in ledger["releases"]
        if entry.get("application_version") == "0.23.2" and entry.get("state") == "finalized"
    ]
    assert "**Current state:" in documentation
    assert "not deployed" in documentation
    assert len(finalized_023) == 1
    assert len(finalized_0231) == 1
    assert len(finalized_0232) == 1
    assert finalized_0231[0].get("task_id") == "codex-project-mirrorline-exhaustive-parity-20260727"
    assert finalized_0232[0].get("task_id") == "codex-project-mirrorline-composition-corrections-20260727"
    assert read_canonical_version(ROOT) >= Version.parse("0.23.2")
